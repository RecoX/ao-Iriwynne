...
'Devuelve el pr�ximo slot de quest libre.
    Dim i As Integer
...
'Maneja el evento de aceptar una quest.
    ...
    'Est� el personaje en la distancia correcta?
    If Distancia(UserList(UserIndex).Pos, Npclist(NpcIndex).Pos) > 5 Then
        Call WriteConsoleMsg(UserIndex, "Est�s demasiado lejos.", FontTypeNames.FONTTYPE_INFO)   '"Est�s demasiado lejos."
        Exit Sub
    End If
    ...
    With UserList(UserIndex).QuestStats.Quests(QuestSlot)
        ...
        Call WriteConsoleMsg(UserIndex, "Has aceptado la misi�n " & Chr(34) & QuestList(.QuestIndex).Nombre & Chr(34) & ".", FontTypeNames.FONTTYPE_INFO)
...
'Maneja el evento de terminar una quest.
    ...
    With QuestList(QuestIndex)
        ...
        If .RequiredKills > 0 Then
            If UserList(UserIndex).flags.RequiredKills < .RequiredKills Then
                Exit Sub
            End If
        End If
        ...
        If .RewardOBJs > 0 Then
            ...
            For i = 1 To getMaxInventorySlots(UserIndex)
            ...
            If InvSlotsLibres < .RewardOBJs Then
                Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("No tienes suficiente espacio en el inventario para recibir la recompensa. Vuelve cuando hayas hecho m�s espacio.", Npclist(NpcIndex).Char.CharIndex, vbWhite))
                Exit Sub
            End If
        End If

        If .RewardPoints > 0 Then
            UserList(UserIndex).flags.PuntosShop = UserList(UserIndex).flags.PuntosShop + .RewardPoints
            'WriteSendPoints UserIndex
        End If
        
        'A esta altura ya cumpli� los objetivos, entonces se le entregan las recompensas.
        Call WriteConsoleMsg(UserIndex, "�Has completado la misi�n " & Chr(34) & QuestList(QuestIndex).Nombre & Chr(34) & "!", FontTypeNames.FONTTYPE_INFO)

        If .AvisaConsola <> 0 Then
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsgNew("Quests", .AvisaConsolaMsg))
        End If

        'Si la quest ped�a objetos, se los saca al personaje.
...
'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    'Est� el personaje en la distancia correcta?
    If Distancia(UserList(UserIndex).Pos, Npclist(NpcIndex).Pos) > 5 Then
        Call WriteConsoleMsg(UserIndex, "Est�s demasiado lejos.", FontTypeNames.FONTTYPE_INFO) ' eMensajes.Mensaje011)    '"Est�s demasiado lejos."
        Exit Sub
    End If

    'El NPC hace quests?
    If Npclist(NpcIndex).QuestNumber = 0 Then
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("No tengo ninguna misi�n para ti.", Npclist(NpcIndex).Char.CharIndex, vbWhite))
        Exit Sub
    End If

    'El personaje ya hizo la quest?
    If UserDoneQuest(UserIndex, Npclist(NpcIndex).QuestNumber) Then
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("Ya has hecho una misi�n para mi.", Npclist(NpcIndex).Char.CharIndex, vbWhite))
        Exit Sub
    End If
    ...
    If UserList(UserIndex).Stats.ELV < QuestList(Npclist(NpcIndex).QuestNumber).RequiredLevel Then
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("Debes ser por lo menos nivel " & QuestList(Npclist(NpcIndex).QuestNumber).RequiredLevel & " para emprender esta misi�n.", Npclist(NpcIndex).Char.CharIndex, vbWhite))
        Exit Sub
    End If
    ...
    If tmpByte Then
        'El usuario est� haciendo la quest, entonces va a hablar con el NPC para recibir la recompensa.
        Call FinishQuest(UserIndex, Npclist(NpcIndex).QuestNumber, tmpByte)
    Else
        'El usuario no est� haciendo la quest, entonces primero recibe un informe con los detalles de la misi�n.
        tmpByte = FreeQuestSlot(UserIndex)

        'El personaje tiene algun slot de quest para la nueva quest?
        If tmpByte = 0 Then
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("Est�s haciendo demasiadas misiones. Vuelve cuando hayas completado alguna.", Npclist(NpcIndex).Char.CharIndex, vbWhite))
            Exit Sub
        End If
...
'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    For i = 1 To NumQuests
        With QuestList(i)
            ...
            .AvisaConsolaMsg = val(Reader.GetValue("QUEST" & i, "AvisaConsolaMsg"))
