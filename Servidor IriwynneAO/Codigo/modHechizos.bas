    ...
    With UserList(UserIndex)
        ...
        ElseIf Hechizos(Spell).SubeHP = 2 Then
            ...
            If .flags.Privilegios And PlayerType.User Then
                ...
                SendData SendTarget.ToPCArea, UserIndex, PrepareMessageCreateDamage(.pos.X, .pos.Y, da�o, DAMAGE_NORMAL)
        ...
        If Hechizos(Spell).Paraliza = 1 Or Hechizos(Spell).Inmoviliza = 1 Then
            ...
            If GranPoder = UserIndex Then Exit Sub
            ...
            If .flags.Paralizado = 0 Then
                ...
                If .Stats.UserAtributos(eAtributos.Agilidad) = MAXATRIBUTOS Then
                    If .Stats.UserAtributos(eAtributos.Fuerza) = MAXATRIBUTOS Then
                        If .Invent.EscudoEqpObjIndex = 0 And .Invent.EscudoEqpSlot = 0 Then
                            ' 1162, arco 1163
                            If .clase = eClass.Warrior Then
                                If .Invent.WeaponEqpObjIndex = 1162 Then
                                    .Stats.UserAtributos(eAtributos.Fuerza) = .Stats.UserAtributos(eAtributos.Fuerza) - 5
                                    Exit Sub
                                End If
                            ElseIf .clase = eClass.Hunter Then
                                If .Invent.WeaponEqpObjIndex = 1163 Then
                                    .Stats.UserAtributos(eAtributos.Fuerza) = .Stats.UserAtributos(eAtributos.Fuerza) - 5
                                    Exit Sub
                                End If
                            End If
...
Sub HechizoInvocacion(ByVal UserIndex As Integer, ByRef HechizoCasteado As Boolean)
        ...
        With UserList(UserIndex)
                ...
                If .Death Then HechizoCasteado = False: Exit Sub
                
                If .flags.Automatico Then HechizoCasteado = False: Exit Sub
                
                If (.sReto.reto_Index <> 0) Or (.mReto.reto_Index <> 0) Then
                        'fERROR = "Est�s en Reto, no puede ocultarte"
                        Call WriteConsoleMsg(UserIndex, "�Estas en Reto, no puedes realizar invocaciones durante el mismo!", FontTypeNames.FONTTYPE_INFO)
                ...
                If MapInfo(Mapa).InvocarSinEfecto = 1 Then
                        Call WriteConsoleMsg(UserIndex, "Invocar no est� permitido aqu�! Retirate de la Zona si deseas utilizar el Hechizo.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If Hechizos(HechizoIndex).Invisibilidad = 1 Then
            ...
            If (.sReto.reto_Index <> 0) Or (.mReto.reto_Index <> 0) Then
                'fERROR = "Est�s en Reto, no puede ocultarte"
                Call WriteConsoleMsg(UserIndex, "�Estas en Reto, no puedes tirar el hechizos invisibilidad Durante el mismo!", FontTypeNames.FONTTYPE_INFO)
                HechizoCasteado = False
                Exit Sub
            ...
            If TargetIndex = GranPoder Then
                Call WriteConsoleMsg(UserIndex, "�No puedes lanzar invisibilidad a �ste usuario!", FontTypeNames.FONTTYPE_INFO)
                HechizoCasteado = False
                Exit Sub
            End If
            ...
            If UserList(TargetIndex).Counters.Invisibilidad <= 5 Then
            
            WriteInviSegundos TargetIndex, 15
            End If
...
Exit Sub
        ...
        If Hechizos(HechizoIndex).Paraliza = 1 Or Hechizos(HechizoIndex).Inmoviliza = 1 Then
            ...
            If GranPoder = TargetIndex Then
                WriteConsoleMsg UserIndex, "No puedes inmovilizar a ese usuario.", FontTypeNames.FONTTYPE_DIOS
            ...
            If UserList(TargetIndex).flags.Paralizado = 0 Then
                ...
                If UserList(TargetIndex).Stats.UserAtributos(eAtributos.Agilidad) = MAXATRIBUTOS Then
                    If UserList(TargetIndex).Stats.UserAtributos(eAtributos.Fuerza) = MAXATRIBUTOS Then
                        If UserList(TargetIndex).Invent.EscudoEqpObjIndex = 0 And UserList(TargetIndex).Invent.EscudoEqpSlot = 0 Then
                            ' 1162, arco 1163
                            If UserList(TargetIndex).clase = eClass.Warrior Then
                                If UserList(TargetIndex).Invent.WeaponEqpObjIndex = 1162 Then
                                    UserList(TargetIndex).Stats.UserAtributos(eAtributos.Fuerza) = UserList(TargetIndex).Stats.UserAtributos(eAtributos.Fuerza) - 5
                                    Call FlushBuffer(TargetIndex)
                                   WriteConsoleMsg TargetIndex, "Berseker ha dejado de funcionar. Dopate rapido para que �ste funcione.", FontTypeNames.FONTTYPE_INFO
                                    Exit Sub
                                End If
                            ElseIf UserList(TargetIndex).clase = eClass.Hunter Then
                                If UserList(TargetIndex).Invent.WeaponEqpObjIndex = 1163 Then
                                    UserList(TargetIndex).Stats.UserAtributos(eAtributos.Fuerza) = UserList(TargetIndex).Stats.UserAtributos(eAtributos.Fuerza) - 5
                                   WriteConsoleMsg TargetIndex, "Berseker ha dejado de funcionar. Dopate rapido para que �ste funcione.", FontTypeNames.FONTTYPE_INFO
                                    Call FlushBuffer(TargetIndex)
        ...
        If Hechizos(HechizoIndex).Revivir = 1 Then
            If UserList(TargetIndex).flags.Muerto = 1 Then
                ...
                ElseIf .clase = eClass.Bard Then
                    ...
                    If .Invent.AnilloEqpObjIndex <> LAUDELFICO And .Invent.AnilloEqpObjIndex <> LAUDMAGICO And .Invent.AnilloEqpObjIndex <> LAUDSHOP Then
                ...
                ElseIf .clase = eClass.Druid Then
                    ...
                    If .Invent.AnilloEqpObjIndex <> FLAUTAELFICA And .Invent.AnilloEqpObjIndex <> FLAUTAMAGICA And .Invent.AnilloEqpObjIndex <> FLAUTASHOP Then
                        Call WriteConsoleMsg(UserIndex, "Necesitas un instrumento m�gico para devolver la vida.", FontTypeNames.FONTTYPE_INFO)
                        HechizoCasteado = False
                        Exit Sub
...
Sub HechizoEstadoNPC(ByVal NpcIndex As Integer, _
        ...
        With Npclist(NpcIndex)
                ...
                If Hechizos(SpellIndex).Invisibilidad = 1 Then
                        Call InfoHechizo(UserIndex)
                        .flags.invisible = 1
                        WriteInviSegundos UserIndex, 15
                        HechizoCasteado = True
...
Sub HechizoPropNPC(ByVal SpellIndex As Integer, _
        ...
        With Npclist(NpcIndex)
                ...
                ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
                        ...
                        SendData SendTarget.ToNPCArea, NpcIndex, Protocol.PrepareMessageCharacterUpdateHP(.Char.CharIndex, .Stats.MinHp, .Stats.MaxHP)
                        SendData SendTarget.ToNPCArea, NpcIndex, PrepareMessageCreateDamage(.pos.X, .pos.Y, da�o, DAMAGE_NORMAL)
        ...
        With UserList(UserIndex)
                ...
                If tUser > 0 Then
                        If UserIndex <> tUser Then
                                If .showName And .Death = False And .EnJDH = False Then
                                ...
                                If .Death = False And .EnJDH = False Then
                                Call WriteConsoleMsg(tUser, .Name & " " & Hechizos(SpellIndex).TargetMsg, FontTypeNames.FONTTYPE_FIGHT)
                                Else
                                Call WriteConsoleMsg(tUser, "Alguien " & Hechizos(SpellIndex).TargetMsg, FontTypeNames.FONTTYPE_FIGHT)
                                End If
                        Else
...
Public Function HechizoPropUsuario(ByVal UserIndex As Integer) As Boolean
        ...
        With UserList(TargetIndex)
                ...
                If Hechizos(SpellIndex).SubeHam = 1 Then
                        ...
                        If UserIndex <> TargetIndex Then
                                Call WriteConsoleMsg(UserIndex, "Le has restaurado " & da�o & " puntos de hambre a " & IIf(.Death, "un participante", .Name) & ".", FontTypeNames.FONTTYPE_FIGHT)
                                Call WriteConsoleMsg(TargetIndex, IIf(.Death, "Un participante", UserList(UserIndex).Name) & " te ha restaurado " & da�o & " puntos de hambre.", FontTypeNames.FONTTYPE_FIGHT)
                        Else
                ...
                If Hechizos(SpellIndex).SubeHP = 1 Then
                        ...
                        If UserIndex <> TargetIndex Then
                                If .Death Then
                                    Call WriteConsoleMsg(UserIndex, "Le has restaurado " & da�o & " puntos de vida a un jugador.", FontTypeNames.FONTTYPE_FIGHT)
                                    Call WriteConsoleMsg(TargetIndex, "Un jugador te ha restaurado " & da�o & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
                                Else
                ...
                ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
                        ...
                        If GranPoder = UserIndex Then da�o = Int(da�o * 1.05)
                        ...
                        SendData SendTarget.ToPCArea, TargetIndex, PrepareMessageCreateDamage(.pos.X, .pos.Y, da�o, DAMAGE_NORMAL)
        
                        If .Death Then
                        Call WriteConsoleMsg(UserIndex, "Le has quitado " & da�o & " puntos de vida a un jugador.", FontTypeNames.FONTTYPE_FIGHT)
                        Call WriteConsoleMsg(TargetIndex, "Un jugador te ha quitado " & da�o & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
                        Else
                ...
                Call WarpUserChar(TargetIndex, UserList(TargetIndex).pos.Map, UserList(TargetIndex).pos.X, UserList(TargetIndex).pos.Y, False, True)
                
                
                                    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsgNew("Favor de los dioses>", UserList(TargetIndex).Name & " ha sido asesinado , ahora " & UserList(UserIndex).Name & " es el nuevo poseedor.", FontTypeNames.FONTTYPE_AMARILLO, FontTypeNames.FONTTYPE_BLANCO))
                                End If
                                End If
                ...
                If Hechizos(SpellIndex).SubeMana = 1 Then
                        ...
                        If UserIndex <> TargetIndex Then
                                If .Death Then
                                Call WriteConsoleMsg(UserIndex, "Le has restaurado " & da�o & " puntos de man� a un Participante.", FontTypeNames.FONTTYPE_FIGHT)
                                Call WriteConsoleMsg(TargetIndex, "Un participante te ha restaurado " & da�o & " puntos de man�.", FontTypeNames.FONTTYPE_FIGHT)
                                Else
...
Public Function CanSupportUser(ByVal CasterIndex As Integer, _
        ...
        With UserList(CasterIndex)
                ...
                If UserList(CasterIndex).pos.Map = Castillo.Mapa Then
                        CanSupportUser = True
                        Exit Function
                End If
                ...
                If .Death Then
                Call WriteConsoleMsg(CasterIndex, "Est�s en death!", FontTypeNames.FONTTYPE_FIGHT)
                Exit Function
...
Public Function CanSupportNpc(ByVal CasterIndex As Integer, _
        ...
        With UserList(CasterIndex)
                ...
                ' Si estas en la arena, esta todo permitido
                If .pos.Map = Castillo.Mapa Or TriggerZonaPelea(CasterIndex, OwnerIndex) = TRIGGER6_PERMITE Then
                        CanSupportNpc = True
                        Exit Function
