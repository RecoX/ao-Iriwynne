...
'********************************************************
    ...
    ' Es pretoriano?
11  If MiNPC.NPCtype = eNPCType.Pretoriano Then

13      If MiNPC.Numero = 901 Or MiNPC.Numero = 900 Then    '@Rey pretoriano
15          ClanConquistaCastillo UserList(UserIndex).GuildIndex
14      End If

55      If MiNPC.ClanIndex > 0 Then
12          Call ClanPretoriano(MiNPC.ClanIndex).MuerePretoriano(NpcIndex)
56      End If

    End If

    'Quitamos el npc
17  Call QuitarNPC(NpcIndex)
        ...
        With UserList(UserIndex)
            ...
            If MiNPC.flags.ExpCount > 0 Then
                ...
                Else
                    ...
                    If .flags.Vip > 0 Then
                        .Stats.Exp = .Stats.Exp + (MiNPC.flags.ExpCount * IIf(HappyHour, 4, 2))
                    Else
                        .Stats.Exp = .Stats.Exp + (MiNPC.flags.ExpCount * IIf(HappyHour, 2, 1))
                    End If
                    ...
                    If .flags.Vip > 0 Then
                        Call WriteConsoleMsg(UserIndex, "Has ganado " & IIf(HappyHour, MiNPC.flags.ExpCount * 4, MiNPC.flags.ExpCount * 2) & " puntos de experiencia. [EXPERIENCIA X2]", FontTypeNames.FONTTYPE_NARANJA)
                    Else
                        Call WriteConsoleMsg(UserIndex, "Has ganado " & IIf(HappyHour, MiNPC.flags.ExpCount * 2, MiNPC.flags.ExpCount) & " puntos de experiencia.", FontTypeNames.FONTTYPE_NARANJA)
                    End If
                End If
...
199:
    ...
    ' @@ Si no lo mato un usuario entonces nop
    If UserIndex > 0 Then
...
errhandler:
    Call LogError("Error en MuereNpc - Error: " & Err.Number & " - Desc: " & Err.description & " ___> " & Erl)
...
Private Sub EraseNPCChar(ByVal NpcIndex As Integer)
        ...
        If Extra.isNPCResucitador(NpcIndex) Then
                Call Extra.DeleteAreaResuTheNpc(NpcIndex)
                        ...
                        If UserIndex > 0 Then
                                ...
                                With UserList(UserIndex)
                                        ...
                                        If Not (.flags.AdminInvisible = 1) Then
                                                ...
                                                'Los valores de visible o invisible est�n invertidos porque estos flags son del NpcIndex, por lo tanto si el npc entra, el casper sale y viceversa :P
...
Public Function OpenNPC(ByVal NpcNumber As Integer, _
        ...
        With Npclist(NpcIndex)
                ...
                .GiveEXP = val(Leer.GetValue("NPC" & NpcNumber, "GiveEXP")) * Configuracion.Experiencia
                ...
                .GiveGLD = val(Leer.GetValue("NPC" & NpcNumber, "GiveGLD")) * Configuracion.Oro
