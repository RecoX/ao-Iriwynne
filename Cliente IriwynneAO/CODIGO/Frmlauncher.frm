...
'EnProceso = True
'Analizar 'Iniciamos la funci�n Analizar =).
...
Function Analizar()
    ...
    'LINK1            'Variable que contiene el numero de actualizaci�n correcto del servidor
    ix = Inet1.OpenURL("http://iriwynneupdater.ucoz.es/VEREXE.txt")
    Call addConsole("Listo.", 1, 255, 1, True, False)
    
    If ix < tx Then
        Call GuardarInt(App.path & "/INIT/Update.ini", "0")
        Call addConsole("Se resetearon las actualizaciones, se repetir� el proceso!", 255, 1, 1, True, False)
        ix = 0
        tx = 0
        Analizar
        Exit Function
...
noanda:
    ...
    Call addConsole("El autoupdater tir� error : " & MiError, 255, 255, 255, True, False)
    Err.Clear
    ix = tx
GoTo 99
...
99    Debug.Print "No hubo error."
  On Error GoTo avisar
    ...
    Label1.Caption = ix
    'Variable que contiene el numero de actualizaci�n del cliente
    ...
    Call addConsole("El cliente ya est� listo para jugar", 255, 255, 255, True, True)  '>> Informacion
...
End Function
        ...
        Public Sub AutoDownload(Numero As Integer)
            ...
            'Informacion
            Call addConsole("   Instalando actualizaci�n.", 0, 100, 255, False, False)    '>> Informacion
